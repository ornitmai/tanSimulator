...
const Simulator3D = (() => {
  ...
  function takeSample() {
    if (!App.state.sim.isPowerOn) return;

    const angleText = document.getElementById('simValAngle').textContent;
    const bText = document.getElementById('simValBLoop').textContent;
    const a = parseFloat(angleText);
    const b = parseFloat(bText);
    const t = Math.tan(a * Math.PI / 180);

    App.state.sim.history.unshift({
      i: App.state.sim.current,
      a: a,
      t: t,
      b: b
    });
  ...
  function renderSimTable() {
    ...
    body.innerHTML = hist.map((s, i) =>
      `<tr class="border-b hover:bg-green-50/30">
        <td class="p-2 border-l">${s.i.toFixed(2)}A</td>
        <td class="p-2 border-l font-bold text-blue-600">${s.a.toFixed(1)}°</td>
        <td class="p-2 border-l font-mono">${s.t.toFixed(3)}</td>
